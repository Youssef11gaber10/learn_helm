...
// Configuration
const port = process.env.PORT || 3000; // this app will run on it 
const secretKey = process.env.SECRET_KEY || 'xco0sr0fh4e52x03g9mv';// we give this to auth to sign jwt ,here we use it to verify jwt , the front make sure user have token before call any service
...
function authenticateToken(req, res, next) {
    ...
    jwt.verify(token, secretKey, function (err, decoded) { //when he make request he verify its token 
      if (err) {
        logger.error(err);
        return res.redirect('/login');
      }
